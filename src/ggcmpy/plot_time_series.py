...
dir_input = Path("/mnt/lustre/germaschewski/cu1010/runs/250925_1220_c034/inp")
dir_cl_observed = Path("/mnt/lustre/germaschewski/cu1010/runs/250925_1220_c034/plots")
dir_cl_model0 = Path("/mnt/lustre/germaschewski/cu1010/runs/raeder/c034")
dir_cl_model = Path("/mnt/lustre/germaschewski/cu1010/runs/250925_1220_c034/target")
...
def _get_cl_model(timestamp: Any, ds: Any, magnetometers: Any):
    ...
    delbt = ggcmpy.iono.iopar(ds).delbt  # type: ignore[no-untyped-call]
    values = []
...
def _get_jh_model(ds: Any):
    """Calculates the model Joule heating for a single timestamp"""
    re = 6371040
    dt = np.deg2rad(180 / (ds.lats.size - 1)).item()
    dp = np.deg2rad(360 / (ds.longs.size - 1)).item()

    xjh = ggcmpy.iono.iopar(ds).xjh  # type: ignore[no-untyped-call]

    xjh_tot = 0.0

    for j in range(int((ds.lats.size - 1) / 2)):
        for i in range(ds.longs.size - 1):
            xjh_tot = xjh_tot + re**2 * np.sin(
                np.deg2rad(90.0 - ds.lats[j].item())
            ).item() * dt * dp * (xjh[i : i + 2, j : j + 2].sum().item() / 4.0)
    return xjh_tot if xjh_tot else np.nan
...
def load_data_jh_model(dir_model: Any, start_time_ggcm: Any):
    """Loads and processes a time series of model-generated Joule heating data"""
    pattern = re.compile(r"\.iof\.(\d{6})$")
    data_points = []
    ...
    for fname in sorted(pathlib.Path.iterdir(dir_model)):
        ...
        try:
            ...
            jh_model = _get_jh_model(ds)
            if not np.isnan(jh_model):
                # Convert from W to GW.
                converted_val = jh_model / 1e9
                data_points.append((timestamp, converted_val))
        except OSError as e:
            print(f"Skipping file {fname}: {e}")  # noqa: T201

    if not data_points:
        return pd.DataFrame(columns=["jh_model"])

    # Create a dataframe from the collected data points.
    return pd.DataFrame(data_points, columns=["datetime", "jh_model"]).set_index(
        "datetime"
    )
...
def plot(factor: Any):
    ...
    datasets = {
        ...
        # "cl_model0": load_data_cl_model(dir_cl_model0, start_time_ggcm, factor),
        "cl_model": load_data_cl_model(dir_cl_model, start_time_ggcm, factor),
        # "cpcp_model0": load_data_cpcp_model(dir_cl_model0, start_time_ggcm),
        "cpcp_model": load_data_cpcp_model(dir_cl_model, start_time_ggcm),
        # "jh_model0": load_data_jh_model(dir_cl_model0, start_time_ggcm),
        "jh_model": load_data_jh_model(dir_cl_model, start_time_ggcm),
    }
    ...
    fig.suptitle(
        "Geomagnetic Data for the January 10, 1997 Storm (CTIM)",
        fontsize=16,
    )
    ...
    # if not datasets["cl_model0"].empty:
    #     ax3.plot(
    #         datasets["cl_model0"]["hours"],
    #         datasets["cl_model0"]["cl_model"],
    #         label="Model CL (Old OpenGGCM)",
    #         color="red",
    #         lw=1,
    #     )
    ...
    ax4 = axes[3]
    # if not datasets["cpcp_model0"].empty:
    #     ax4.plot(
    #         datasets["cpcp_model0"]["hours"],
    #         datasets["cpcp_model0"]["cpcp_model"],
    #         label="Model CPCP (Old OpenGGCM)",
    #         color="red",
    #         lw=1,
    #     )
    ...
    # Plot Panel 4 (Model JH).
    ax5 = axes[4]
    # if not datasets["jh_model0"].empty:
    #     ax5.plot(
    #         datasets["jh_model0"]["hours"],
    #         datasets["jh_model0"]["jh_model"],
    #         label="Model JH (Old OpenGGCM)",
    #         color="red",
    #         lw=1,
    #     )
    if not datasets["jh_model"].empty:
        ax5.plot(
            datasets["jh_model"]["hours"],
            datasets["jh_model"]["jh_model"],
            label="Model JH",
            color="blue",
            lw=1,
        )
    ...
    ax5.set_ylabel("Joule Heating [GW]")
    ax5.set_xlabel(f"Time (hours from {start_time_plot.strftime('%Y-%m-%d %H:%M')} UT)")
    ax5.set_xlim(0, 14)
    ...
    filename = f"geomagnetic_data_{start_time_plot.date()!s}_{dir_par_name}.png"
