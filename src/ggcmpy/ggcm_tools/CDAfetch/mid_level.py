    ...
    class cls(CDA_XML):
        ...
        def __str__(self):
            s = (
                ...
                + "\n\t"
                + "\n\t".join(
                    [(v + ": " + str(getattr(self, v))) for v in self.attr[1:]]
                )
            )
            return s
...
class CDA_XML_Dataview(
    CDA_XML_factory(
        ["EndpointAddress", "Title", "Subtitle", "UnderConstruction", "NoticeUrl"],
        NameAttr="Id",
    )
):
    pass
