    ...
    try:
        ...
        for i in reversed(range(len(l) + 1)):
            ...
            try:
                ...
                pass
    sys.stderr.write("Parse Error: Cannot parse date from "
                     "string: %s\n"%x)
    sys.exit(2)
...
def main():
    ...
    parser.add_argument('-d', '--debug', '--verbose', dest='debug',
                        action='store_true',
                        help="verbose output")
    parser.add_argument('--satellite', '--sat', '-s',
                        dest='sat',
                        default=None,
                        help="Satellite to use (ac,wi,omni,ge ...)")
    parser.add_argument('--fillgap', dest='gap', type=int, default=30,
                        help="Fill gaps at this cadence")
    parser.add_argument('--starttime', '--start', '-S',
                        dest='starttime',
                        type=datetimetype,
                        default=None,
                        help="Start time (YYYY:mm:dd:HH:MM:Secs)")
    parser.add_argument('--endtime', '--end', '-E',
                        dest='endtime',
                        type=datetimetype,
                        default=None,
                        help="End time (YYYY:mm:dd:HH:MM:Secs)")
    parser.add_argument('--dipoletime', '--dip', '--diptime', '-D', dest='diptime',
                        type=datetimetype, default=None,
                        help="dipole time. This is the "
                        "timecode used for --f107 and --mpos")
    parser.add_argument('--f107', dest='f107', default=None, action='store_true',
    ...
    parser.add_argument('-m', '--monitor-position', dest='mopos', default=None,
                        action='store_true',
                        help="Attempt to get monitor position also.")
    parser.add_argument('--no-plot', dest='noplot', default=False,
                        action='store_true', help="Don't plot the input variables")
    parser.add_argument('--red_bzneg', dest='red_bzneg', action='store_true',
    ...
    files = opt.files
    satfetchfuncs = {'ac': fetch.getAceData, 'wi': fetch.getWindData,
                     'omni': fetch.getOMNIData, 'ge': fetch.getGeotailData}

    if not files:  #It looks like we're going to need to download the data
        files = []
        msg = "Must provide start (--starttime), end (--endtime) and satellite " \
        ...
        try:
            ...
            parser.error("Unrecognized satellite [%s] available: %s "
    ...
    for var in CDAdata.keys():
        ...
        CDAdata[var] = CDAdata[var].t_interp(gap=opt.gap,
    ...
    if opt.sat == 'ac' and 'vth' in CDAdata:
        CDAdata['temp'] = CDAdata['vth'].map(lambda x: 60.1366 * x * x * 1.e-6)
    elif 'vth' in CDAdata:
    ...
    if opt.plot:
        try:
            ...
            sys.stderr.write("Tried to plot, but numpy / matplotlib are not "
