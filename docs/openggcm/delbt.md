...
ground magnetic perturbation caused by substorm electrojets, $\delta B_\theta$, as follows:
...
- `iopar` solves a Poisson equation using the `io4` subroutine in the `io-psol.for` file to obtain the poloidal current density potential, `tau`,
...
- Computes the cross-product of the current density vector (`xjp`=`cpolp`, `xjt`=`cpolt`) and the position vector to get the magnetic field in Cartesian coordinates (`dbx`, `dby`, `dbz`)
