subroutine inquire_next(IU, found_field, ndim, nx, ny, nz, it, c_varname, c_tstring) bind(c) !found_field,IU,ndim,nx,ny,nz,it,varname,tstring)
   ...
   use c_string
   implicit none
   integer(kind=C_INT), value, intent(in) :: IU
   integer(kind=C_INT), intent(out) :: found_field
   integer(kind=C_INT), intent(out) :: ndim, nx, ny, nz, it
   type(C_PTR), intent(inout) :: c_varname, c_tstring
   ...
   backspace(IU)
   backspace(IU)
   c_varname = c_strdup_f(varname)
   c_tstring = c_strdup_f(tstring)
888 continue
